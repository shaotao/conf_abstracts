#!/usr/bin/python3.8

import sys
import requests
from bs4 import BeautifulSoup
import csv
import urllib
import re


# main function starts here, url
# usage: ./crawl_nsdi.py <url>


BASE="https://www.usenix.org"

def get_abstract(link):
    resp = requests.get(link)
    soup = BeautifulSoup(resp.text, 'html.parser');
    item = soup.find('div', {'class':'field field-name-field-paper-description field-type-text-long field-label-above'})
    if (item is None):
        return None;
    abstract_field = item.find('div', {"class":"field-item odd"}).find('p');
    abstract = abstract_field.text.strip() if abstract_field is not None else ""
    return abstract

# main function starts here

if(len(sys.argv) != 2 and len(sys.argv) != 3):
    print("usage: ./crawl_nsdi.py <url> [output_filename]")
    sys.exit(0);

url=sys.argv[1];
output_filename = sys.argv[2] if len(sys.argv)==3 else "data.csv"

print("# crawl site={url}, output_filename={output_filename}...".format(url=url, output_filename=output_filename))
resp=requests.get(url);

parts = urllib.parse.urlparse(resp.url)
BASE= parts.scheme+"://"+parts.netloc;
#print("base = "+BASE)

soup=BeautifulSoup(resp.text, 'html.parser')

with open(output_filename, mode='w') as csv_file:
    writer = csv.writer(csv_file, delimiter=',', quotechar='"');
    for item in soup.find_all('h2', {'class':'node-title'}):
        title = item.find('a')
        if (title == None):
            print("skip item: {text}".format(text=item.text))
            continue;
        link = urllib.parse.urljoin(BASE, title['href'])
        article_title = re.sub("\s+", " ", title.text.strip())
        if (article_title == ""):
            #print("skip item: {item}".format(item=item))
            continue;
        abstract = get_abstract(link)
        if (abstract is None):
            #print("skip item: {item}".format(item=item))
            continue;
        print('title = '+article_title)
        writer.writerow([article_title, abstract])

